...
print(">>>>>>>>> Filtering 5 label with both filter <<<<<<<<<<<<")
os.makedirs(r'.\pipeline_both', exist_ok=True)
...
print(">>>>>>>>> Normalize 5 label with both filter <<<<<<<<<<<<")
...
for label_ in raw_data_true:
    ...
    dataset_true[label_] = {}
    temp_data, temp_label = create_dataset(data, label, filter_both, scaler)
...
for label_ in raw_data_false:
    ...
    temp_data, temp_label = create_dataset(data, label, filter_both, scaler, epsilon=0.1)
...
data, input_data = get_input(df_right, filter_both, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_left, filter_both, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_both, filter_both, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_teeth, filter_both, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
