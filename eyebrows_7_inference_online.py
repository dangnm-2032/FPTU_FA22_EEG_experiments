...
##################### MODEL ###############################################
model = load_model(r'.\pipeline_eyebrows\checkpoints\checkpoint.keras')
###########################################################################


##################### SCALER ##############################################
import joblib
scaler = joblib.load(r'.\pipeline_eyebrows\checkpoints\scaler.save')  
###########################################################################
...
while True:
    ...
    ########################### Preprocessing ###################################
    x_eyebrows = pipeline(eeg_data, filter_eyebrows, scaler)
    
    input = np.concatenate([x_eyebrows], axis=1)
